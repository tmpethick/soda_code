...
# -----------------------------------------------------------------------------
# SODAWrapper wraps Adam after the warmup phase.

class SODAWrapper_SkipWarmup(torch.optim.Optimizer):
    r"""
    OBS: Do not use weight decay for the underlying optimizer.
    """
    def __init__(self, base : torch.optim.Optimizer, warmup_iters=0):
        self.base = base
...
optimizer1 = torch.optim.AdamW(raw_model.lm_head.parameters(), lr=2*args.learning_rate, betas=(0.9, 0.95),
                            weight_decay=0, fused=True)
optimizer2 = torch.optim.AdamW(raw_model.transformer.h.parameters(), lr=args.learning_rate, betas=(0.9, 0.95),
                            weight_decay=0, fused=True)
optimizer1, optimizer2 = optimizer1, SODAWrapper_SkipWarmup(optimizer2, warmup_iters=args.warmup_iters)
